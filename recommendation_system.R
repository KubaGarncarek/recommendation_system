...
# extract year of production from title and creating new column - year_of_prod

edx <- edx %>% mutate(year_of_prod = str_extract(title, pattern= ".\\d{4}.$")) %>%
  mutate(year_of_prod = str_extract(year_of_prod, pattern = "\\d{4}"))

# add rate date column
edx <- edx %>% mutate(rate_date = as_datetime(timestamp))
edx$rate_date <- round_date(edx$rate_date, unit="month")


#spit edx into train and test sets
...
# proportion of users that give number of rates more than average
...
top_movie %>%
  ...
  summarize(unique(title), unique(movieId), unique(mean_rate))
...
######### 3.4 Production year analysis ##########

#Create object years_rates which contain number of rate and mean rate which received
#movies created in each year and number of this movies 
...
# Number of best movies in each year
...
# Plot annual number of best movies versus year
...
tibble(Model_Type = c("mean base model"), 
                       RMSE = c(mean_m1))%>%
...
tibble(Model_Type = c("mean base model", "Movie effect model"), 
...
# Calculate effect genres combo by mean of thier singles effect
...
rm(movie_avgs, user_avgs, year_avgs, rate_date_avgs, genres_avgs, genres_combo_avgs)
...
tibble(Model_Type = c("mean base model", "Movie effects model",
